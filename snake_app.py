...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def next_generation(self):
        ...
        self.save_model(filename=f"best_model_gen_{self.current_generation}.pkl")
    ...
    def save_model(self, filename="best_snake_model.pkl"):
        """Saves the best snake's model to a file."""
        with open(filename, 'wb') as f:
            pickle.dump(self.population.fittest_individual.network.params, f)
        print(f"Model saved to {filename}")


    
    def load_model(self, filename="best_snake_model.pkl"):
        """Loads a saved snake model from a file."""
        try:
            with open(filename, 'rb') as f:
                loaded_params = pickle.load(f)
                # Create a new snake with the loaded parameters
                loaded_snake = Snake(self.board_size, chromosome=loaded_params,
